...
void VoxelGridFilter::myFilter(PointCloudT &output)
{
  if (!initCompute())
    return;
...
void VoxelGridFilter::myApplyFilter(PointCloudT &output)
{
  ...
  if (!filter_field_name_.empty ()) // If we don't want to process the entire cloud...
    pcl::getMinMax3D<PointNT> (input_, *indices_, filter_field_name_, static_cast<float> (filter_limit_min_), static_cast<float> (filter_limit_max_), min_p, max_p, filter_limit_negative_);
  else
    pcl::getMinMax3D<PointNT> (*input_, *indices_, min_p, max_p);
  ...
  if (save_leaf_layout_)
  {
    ...
    {
      std::cout << "VoxelGrid bin size is too low; impossible to allocate memory for layout" << std::endl;
      //throw PCLException("VoxelGrid bin size is too low; impossible to allocate memory for layout", "voxel_grid.hpp", "applyFilter");
    }
  ...
  for (unsigned int cp = 0; cp < first_and_last_indices_vector.size (); ++cp)
  {
    ...
    {
      pcl::CentroidPoint<PointNT> centroid;
...
void FeatureEstimationOMP::computeFeature(PointCloudOut &output)
{
